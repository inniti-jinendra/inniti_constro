...
class _SelfAttendanceScreenState extends ConsumerState<SelfAttendanceScreen> {
  ...
  Future<void> _init() async {
    AppLogger.info("🔄 Initializing attendance screen...");
  ...
  Future<void> _fetchAttendanceData() async {
    AppLogger.info("📡 Initiating fetch of self-attendance data...");
    setState(() {
      _isLoading = true;
    });
    ...
    try {
      ...
      AppLogger.info(
          "✅ Self-attendance data fetched successfully. Count: ${data.length}");
      ...
      if (data.isNotEmpty) {
        ...
        // Log specific fields from the first attendance record
        AppLogger.info(
            "🔍 First attendance record - empAttendanceId: ${firstAttendanceRecord.empAttendanceId}");
        AppLogger.info(
            "🔍 First attendance record - inTime: ${firstAttendanceRecord.inTime}");
        AppLogger.info(
            "🔍 First attendance record - outTime: ${firstAttendanceRecord.outTime}");
      ...
      _error = e.toString();
      AppLogger.error(
          "❌ Exception occurred while fetching attendance data: $_error");
    } finally {
      setState(() {
        _isLoading = false;
      });
  ...
  Future<void> _refreshAttendanceData() async {
    ...
    try {
      final data = await AttendanceApiService.fetchSelfAttendanceData();
      AppLogger.info(
          "✅ Self-attendance data fetched (refresh). Count: ${data.length}");
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              //     null) // Show total attendance count if available
              //   Padding(
...
class AttendanceSummaryCard extends StatelessWidget {
  ...
  final int? totalAttendanceCount; // Added total attendance count
  ...
  @override
  Widget build(BuildContext context) {
    // If attendance data is null, show a loading indicator
    ...
    //     ),
    //     SizedBox(height: 8.h),
    //     if (totalAttendanceCount != null) // Show total attendance count if available
    //       Padding(
    //         padding: EdgeInsets.only(bottom: 8.h),
    //         child: Text(
